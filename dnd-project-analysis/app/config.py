...
def load_settings() -> Settings:
    ...
    return Settings(
        ...
        log_level=getenv('LOG_LEVEL', 'INFO').strip().upper() or 'INFO',
        redis_url=getenv('REDIS_URL', 'redis://redis:6379/0'),
        backup_dir=getenv('BACKUP_DIR', '/app/data/backups'),
        backup_interval_hours=max(1, int(getenv('BACKUP_INTERVAL_HOURS', '24'))),
        backup_retention_days=max(1, int(getenv('BACKUP_RETENTION_DAYS', '14'))),
    )
