...
from app.gameplay import format_condition_details, format_conditions, format_initiative, format_roll, format_session_card, roll_dice
...
def character_sheet_text(character: dict, sheet: dict) -> str:
    class_line = ' '.join(part for part in [sheet.get('class_name'), sheet.get('subclass_name')] if part)
    return (
        f'📚 <b>Лист персонажа: {html.escape(character["display_name"])}</b>\n\n'
        f'Игрок: <b>{html.escape(sheet.get("player_name") or "не указан")}</b>\n'
        f'Класс: <b>{html.escape(class_line or "не указан")}</b>\n'
        f'Уровень: <b>{int(sheet.get("level") or 1)}</b>\n'
        f'Раса: <b>{html.escape(sheet.get("race_name") or "не указана")}</b>\n'
        f'Предыстория: <b>{html.escape(sheet.get("background_name") or "не указана")}</b>\n'
        f'Мировоззрение: <b>{html.escape(sheet.get("alignment") or "не указано")}</b>'
    )


def character_sheet_entry_text(entry: dict) -> str:
    title = SECTION_LABELS.get(entry['section'], entry['section'])
    description = html.escape(str(entry.get('description') or 'Описание пока не добавлено.'))
    if len(description) > 3400:
        description = description[:3397] + '...'
...
async def profile_command(message: Message, db: Database) -> None:
    ...
    if character is None:
        return
    encounter = await db.get_character_active_initiative(int(character['id']))
    await message.answer(
        await profile_text(character, db),
        reply_markup=player_profile_keyboard(character, bool(encounter)),
    )
...
@router.callback_query(F.data == 'player:sheet')
async def player_character_sheet(callback: CallbackQuery, db: Database) -> None:
    character = await require_character(callback, db)
    if character is None:
        return
    sheet = await db.get_character_sheet(int(character['id']))
    if sheet is None:
    ...
    await edit_or_answer(
        ...
        reply_markup=character_sheet_sections_keyboard(sheet['section_counts'], 'player'),
    )
    await callback.answer()


@router.callback_query(F.data.startswith('player:sheet_section:'))
async def player_character_sheet_section(callback: CallbackQuery, db: Database) -> None:
    character = await require_character(callback, db)
    if character is None:
        return
    section = callback.data.split(':', 2)[2]
    entries = await db.list_character_sheet_entries(int(character['id']), section)
    title = SECTION_LABELS.get(section, 'Раздел')
    await edit_or_answer(
        callback.message,
        f'📚 <b>{html.escape(title)}</b>\n\nВыбери запись:',
        reply_markup=character_sheet_entries_keyboard(entries, 'player', section),
    )
    await callback.answer()


@router.callback_query(F.data.startswith('player:sheet_entry:'))
async def player_character_sheet_entry(callback: CallbackQuery, db: Database) -> None:
    character = await require_character(callback, db)
    if character is None:
        return
    entry = await db.get_character_sheet_entry(int(character['id']), int(callback.data.rsplit(':', 1)[1]))
    if entry is None:
    ...
    await edit_or_answer(
        ...
        reply_markup=character_sheet_entry_keyboard('player', entry['section']),
    )
    await callback.answer()


@router.message(Command('session'))
async def session_card_command(message: Message, db: Database) -> None:
    character = await require_character(message, db)
    if character is None:
        return
    session = await db.get_session(int(character['session_id'])) if character.get('session_id') is not None else None
    text = format_session_card(session) if session else 'У персонажа пока нет сессии.'
    await message.answer(text, reply_markup=session_card_keyboard())


@router.callback_query(F.data == 'player:session_card')
async def session_card_callback(callback: CallbackQuery, db: Database) -> None:
    character = await require_character(callback, db)
    if character is None:
        return
    session = await db.get_session(int(character['session_id'])) if character.get('session_id') is not None else None
...
@router.callback_query(F.data == 'player:spend_inspiration')
async def spend_inspiration_callback(callback: CallbackQuery, db: Database) -> None:
    character = await require_character(callback, db)
    if character is None:
        return
    success, result = await db.spend_inspiration(int(character['id']))
    updated = await db.get_character(int(character['id']))
    encounter = await db.get_character_active_initiative(int(character['id']))
    if updated:
        await edit_or_answer(
            callback.message,
            await profile_text(updated, db),
            reply_markup=player_profile_keyboard(updated, bool(encounter)),
        )
    await callback.answer(result, show_alert=not success)


@router.message(Command('initiative'))
async def initiative_command(message: Message, db: Database) -> None:
    character = await require_character(message, db)
    if character is None:
        return
    encounter = await db.get_character_active_initiative(int(character['id']))
    entry = next(
        (row for row in (encounter or {}).get('entries', []) if int(row['character_id']) == int(character['id'])),
        None,
    )
    can_roll = bool(encounter and entry and entry.get('roll_value') is None)
    await message.answer(format_initiative(encounter), reply_markup=player_initiative_keyboard(can_roll))


@router.callback_query(F.data == 'player:initiative')
async def initiative_callback(callback: CallbackQuery, db: Database) -> None:
    character = await require_character(callback, db)
    if character is None:
        return
    encounter = await db.get_character_active_initiative(int(character['id']))
    ...
    can_roll = bool(encounter and entry and entry.get('roll_value') is None)
    await edit_or_answer(callback.message, format_initiative(encounter), reply_markup=player_initiative_keyboard(can_roll))
    await callback.answer()


@router.callback_query(F.data == 'player:initiative_roll')
async def initiative_roll_callback(callback: CallbackQuery, db: Database) -> None:
    character = await require_character(callback, db)
    if character is None:
        return
    success, result = await db.roll_character_initiative(int(character['id']))
    encounter = await db.get_character_active_initiative(int(character['id']))
    await edit_or_answer(
        callback.message,
        result + '\n\n' + format_initiative(encounter),
        reply_markup=player_initiative_keyboard(False),
    )
    await callback.answer('Бросок готов' if success else result, show_alert=not success)


def _dice_expression_from_message(message: Message) -> str:
    parts = str(message.text or '').split(maxsplit=1)
    return parts[1] if len(parts) > 1 else ''


async def send_dice_result(message: Message, expression: str) -> None:
    try:
    ...
    await message.answer(
        format_roll(result),
        reply_markup=dice_result_keyboard(str(result['expression']), int(result['modifier'])),
    )


@router.message(Command('r', 'roll'))
async def dice_command(message: Message) -> None:
    await send_dice_result(message, _dice_expression_from_message(message))


@router.callback_query(F.data.startswith('dice:roll:'))
async def dice_callback(callback: CallbackQuery) -> None:
    expression = callback.data.split(':', 2)[2]
    try:
        result = roll_dice(expression)
    except ValueError as exc:
...
async def journal_command(message: Message, db: Database) -> None:
    character = await require_character(message, db)
    if character is None:
        return
    entries = await db.list_character_journal(int(character['id']), limit=20)
    await message.answer(journal_text(character, entries), reply_markup=back_to_main_menu())


@router.callback_query(F.data == 'player:journal')
async def journal_callback(callback: CallbackQuery, db: Database) -> None:
    character = await require_character(callback, db)
    if character is None:
        return
    entries = await db.list_character_journal(int(character['id']), limit=20)
    await edit_or_answer(callback.message, journal_text(character, entries), reply_markup=back_to_main_menu())
    await callback.answer()
...
async def use_item_callback(callback: CallbackQuery, db: Database, settings: Settings) -> None:
    character = await require_character(callback, db)
    if character is None:
        return
    item_id = int(callback.data.split(':')[2])
    ...
    for admin_id in settings.admin_ids:
        ...
        try:
            await callback.bot.send_message(
                admin_id,
                f'🧪 <b>{character["display_name"]}</b> использовал: <b>{item_name}</b>.',
            )
        except Exception:
            pass
    carry = await db.get_character_carry_info(character['id'])
