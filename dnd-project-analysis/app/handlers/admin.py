...
from app.assistant_keyboards import condition_duration_keyboard, create_consumable_keyboard, create_equipment_keyboard
from app.database import Database, default_sell_price
from app.character_sheet import SECTION_LABELS
from app.handlers.player import character_sheet_entry_text, character_sheet_text
from app.feature_visibility import visible_journal_entries
from app.gameplay import format_condition_details, format_conditions, format_initiative, format_session_card
...
from app.keyboards import (
    ...
    character_conditions_keyboard,
    character_sheet_entries_keyboard,
    character_sheet_entry_keyboard,
    character_sheet_sections_keyboard,
...
def journal_text(character: dict, entries: list[dict]) -> str:
    entries = visible_journal_entries(entries)
    if not entries:
        return f'📖 <b>Журнал персонажа: {html.escape(character["display_name"])}</b>\n\nЗаписей пока нет.'
    lines = [f'📖 <b>Журнал персонажа: {html.escape(character["display_name"])}</b>']
    for entry in entries:
        created = str(entry.get('created_at') or '')[:16].replace('T', ' ')
        title = html.escape(str(entry.get('title') or 'Запись'))
        details = html.escape(str(entry.get('details') or '').strip())
        details_line = f'\n{details}' if details else ''
        lines.append(f'\n<b>{created}</b> — {title}{details_line}')
    return '\n'.join(lines)
...
async def character_admin_text(character: dict, db: Database) -> str:
    ...
    tg = 'привязан' if character.get('telegram_id') else 'не привязан'
    notes = str(character.get('admin_notes') or '').strip()
    notes_text = html.escape(notes) if notes else 'нет'
    return (
        f'👤 <b>{character["display_name"]}</b>\n'
...
@router.callback_query(F.data == 'admin:characters_menu')
async def admin_characters_menu_callback(callback: CallbackQuery, settings: Settings) -> None:
    if await deny_if_not_admin(callback, settings):
        return
    await edit_or_answer(callback.message, '👥 <b>Персонажи</b>', reply_markup=admin_characters_menu())
    await callback.answer()


@router.callback_query(F.data == 'admin:sessions_menu')
async def admin_sessions_menu_callback(callback: CallbackQuery, settings: Settings) -> None:
    if await deny_if_not_admin(callback, settings):
        return
    await edit_or_answer(callback.message, '🎲 <b>Сессии и отдых</b>', reply_markup=admin_sessions_menu())
    await callback.answer()


@router.callback_query(F.data == 'admin:rewards_menu')
async def admin_rewards_menu_callback(callback: CallbackQuery, settings: Settings) -> None:
    if await deny_if_not_admin(callback, settings):
        return
    await edit_or_answer(callback.message, '📜 <b>Квесты и награды</b>', reply_markup=admin_rewards_menu())
    await callback.answer()


@router.callback_query(F.data == 'admin:items_menu')
async def admin_items_menu_callback(callback: CallbackQuery, settings: Settings) -> None:
    if await deny_if_not_admin(callback, settings):
        return
    await edit_or_answer(callback.message, '📦 <b>Предметы и магазин</b>', reply_markup=admin_items_menu())
    await callback.answer()


@router.callback_query(F.data == 'admin:system_menu')
async def admin_system_menu_callback(callback: CallbackQuery, settings: Settings) -> None:
    if await deny_if_not_admin(callback, settings):
        return
    await edit_or_answer(callback.message, '🛠️ <b>Служебное</b>', reply_markup=admin_system_menu())
    await callback.answer()


@router.callback_query(F.data == 'admin:system_health')
async def admin_system_health(callback: CallbackQuery, db: Database, settings: Settings) -> None:
    if await deny_if_not_admin(callback, settings):
        return
    health = await db.health_summary()
    status = '✅ всё хорошо' if health['integrity'] == 'ok' and health['foreign_key_issues'] == 0 else '⚠️ нужна проверка'
    text = (
        f'🩺 <b>Состояние базы: {status}</b>\n\n'
        f'Целостность: <code>{health["integrity"]}</code>\n'
        f'Ошибки связей: <b>{health["foreign_key_issues"]}</b>\n'
        f'Персонажи: <b>{health["characters"]}</b>\n'
        f'Предметы: <b>{health["items"]}</b>'
    )
    await edit_or_answer(callback.message, text, reply_markup=admin_system_menu())
    await callback.answer()
...
async def admin_session_details(callback: CallbackQuery, db: Database, settings: Settings) -> None:
    ...
    chars_text = '\n'.join(
        f'• <b>{character["display_name"]}</b> | ур. {character.get("level", 1)} '
        f'| XP {character["xp"]} | 🪙 {character["gold"]}'
    ...
    text = (
        ...
        f'<b>Персонажи:</b>\n{chars_text}\n\n'
        'Кнопка «Выдать XP и золото всей сессии» начисляет выбранную награду '
        '<b>каждому</b> персонажу из списка.'
    )
    await edit_or_answer(callback.message, text, reply_markup=session_details_keyboard(session_id))
    await callback.answer()


@router.callback_query(F.data.startswith('admin:session_card:'))
async def admin_session_card_edit_start(
    ...
    if await deny_if_not_admin(callback, settings):
        return
    _, _, session_id_raw, field = callback.data.split(':', 3)
    session_id = int(session_id_raw)
    session = await db.get_session(session_id)
    labels = {
        'next_session_text': 'дату и время следующей игры',
        'current_goal': 'текущую цель группы',
        'last_recap': 'коротко, где остановилась группа',
        'session_note': 'важную заметку для игроков',
    }
    if session is None or field not in labels:
        await callback.answer('Сессия или поле не найдены.', show_alert=True)
        return
    current = str(session.get(field) or '').strip() or 'не указано'
    await state.clear()
    await state.update_data(session_id=session_id, session_card_field=field)
    await state.set_state(SessionCardState.value)
    await callback.message.answer(
        f'Введи {labels[field]}.\n\nСейчас: <i>{html.escape(current)}</i>\n\nЧтобы очистить поле, отправь <code>-</code>.'
    )
    await callback.answer()


@router.message(SessionCardState.value)
async def admin_session_card_edit_finish(
    message: Message,
    state: FSMContext,
    db: Database,
    settings: Settings,
    ...
    field = str(data['session_card_field'])
    value = str(message.text or '').strip()
    if value in {'-', '/clear'}:
        value = ''
    success, result = await db.update_session_card_field(session_id, field, value)
    await state.clear()
    session = await db.get_session(session_id)
    text = result if not success or session is None else format_session_card(session)
    await message.answer(text, reply_markup=session_details_keyboard(session_id))


@router.callback_query(F.data.startswith('admin:session_summary:'))
async def admin_session_summary(callback: CallbackQuery, db: Database, settings: Settings) -> None:
    if await deny_if_not_admin(callback, settings):
        return
    session_id = int(callback.data.split(':')[2])
    session = await db.get_session(session_id)
    ...
    if session is None:
        await callback.answer('Сессия не найдена.', show_alert=True)
        return
    lines = [f'📊 <b>Состояние группы: {html.escape(session["title"])}</b>']
    for character in characters:
        carry = await db.get_character_carry_info(int(character['id']))
        conditions = await db.list_character_conditions(int(character['id']))
        overload = ' | ⚠️ перегруз' if carry and carry.get('overloaded') else ''
        inspiration = ' | ✨ вдохновение' if int(character.get('inspiration') or 0) else ''
        lines.append(
            f'\n• <b>{html.escape(character["display_name"])}</b> — '
            f'HP {int(character.get("hp") or 0)}/{int(character.get("max_hp") or 0)}'
            f'{inspiration}{overload}\n'
            f'  Состояния: {format_conditions(conditions)}'
        )
    if not characters:
        lines.append('\nПерсонажей пока нет.')
    await edit_or_answer(callback.message, '\n'.join(lines), reply_markup=session_details_keyboard(session_id))
    await callback.answer()


@router.callback_query(F.data.startswith('admin:initiative_start:'))
async def admin_initiative_start(callback: CallbackQuery, db: Database, settings: Settings) -> None:
    if await deny_if_not_admin(callback, settings):
        return
    session_id = int(callback.data.split(':')[2])
    success, result, _ = await db.start_initiative(session_id)
    encounter = await db.get_active_initiative(session_id) if success else None
    if success:
        for recipient in await db.list_session_recipients(session_id):
            try:
                await callback.message.bot.send_message(
                    int(recipient['telegram_id']),
                    '⚔️ Мастер запустил инициативу. Нажми кнопку и брось один раз.',
                    reply_markup=player_initiative_keyboard(True),
                )
            except Exception:
                pass
    text = result + ('\n\n' + format_initiative(encounter) if encounter else '')
    reply_markup = admin_initiative_keyboard(int(encounter['id']), session_id) if encounter else session_details_keyboard(session_id)
    await edit_or_answer(callback.message, text, reply_markup=reply_markup)
    await callback.answer()


@router.callback_query(F.data.startswith('admin:initiative_view:'))
async def admin_initiative_view(callback: CallbackQuery, db: Database, settings: Settings) -> None:
    if await deny_if_not_admin(callback, settings):
        return
    session_id = int(callback.data.split(':')[2])
    encounter = await db.get_active_initiative(session_id)
    reply_markup = (
        admin_initiative_keyboard(int(encounter['id']), session_id)
        if encounter else session_details_keyboard(session_id)
    )
    await edit_or_answer(callback.message, format_initiative(encounter), reply_markup=reply_markup)
    await callback.answer()


@router.callback_query(F.data.startswith('admin:initiative_next:'))
async def admin_initiative_next(callback: CallbackQuery, db: Database, settings: Settings) -> None:
    if await deny_if_not_admin(callback, settings):
        return
    _, _, encounter_id_raw, session_id_raw = callback.data.split(':')
    success, result = await db.advance_initiative(int(encounter_id_raw))
    encounter = await db.get_active_initiative(int(session_id_raw))
    text = result + ('\n\n' + format_initiative(encounter) if encounter else '')
    reply_markup = (
        admin_initiative_keyboard(int(encounter['id']), int(session_id_raw))
        if encounter else session_details_keyboard(int(session_id_raw))
    )
    await edit_or_answer(callback.message, text, reply_markup=reply_markup)
    await callback.answer('Готово' if success else result, show_alert=not success)


@router.callback_query(F.data.startswith('admin:initiative_end:'))
async def admin_initiative_end(callback: CallbackQuery, db: Database, settings: Settings) -> None:
    if await deny_if_not_admin(callback, settings):
        return
    _, _, encounter_id_raw, session_id_raw = callback.data.split(':')
    success, result = await db.end_initiative(int(encounter_id_raw))
...
async def admin_notify_session_finish(message: Message, state: FSMContext, db: Database, settings: Settings) -> None:
    ...
    await db.update_session_card_field(session_id, 'next_session_text', str(data.get('date_text') or '')[:600])
    await db.update_session_card_field(session_id, 'session_note', notify_text[:600])
...
async def create_character_password(message: Message, state: FSMContext, settings: Settings) -> None:
    ...
    if len(password) < 6:
        await message.answer('Пароль должен быть минимум 6 символов. Введи другой пароль:')
        return
...
async def characters_list(callback: CallbackQuery, db: Database, settings: Settings) -> None:
    ...
    text = (
        '👥 <b>Персонажи</b>\n\n'
        'Выбери персонажа кнопкой ниже, чтобы открыть карточку и изменить имя, логин, пароль, XP, монеты, Силу или лимит веса.\n\n'
...
@router.callback_query(F.data.startswith('admin:sheet:'))
async def admin_character_sheet(callback: CallbackQuery, db: Database, settings: Settings) -> None:
    if await deny_if_not_admin(callback, settings):
        return
    character_id = int(callback.data.rsplit(':', 1)[1])
    character = await db.get_character(character_id)
    sheet = await db.get_character_sheet(character_id)
    if character is None or sheet is None:
        await callback.answer('Лист персонажа пока не импортирован.', show_alert=True)
        return
    await edit_or_answer(
        callback.message,
        character_sheet_text(character, sheet),
        reply_markup=character_sheet_sections_keyboard(sheet['section_counts'], 'admin', character_id),
    )
    await callback.answer()


@router.callback_query(F.data.startswith('admin:sheet_section:'))
async def admin_character_sheet_section(callback: CallbackQuery, db: Database, settings: Settings) -> None:
    if await deny_if_not_admin(callback, settings):
        return
    _, _, character_id_raw, section = callback.data.split(':', 3)
    character_id = int(character_id_raw)
    entries = await db.list_character_sheet_entries(character_id, section)
    title = SECTION_LABELS.get(section, 'Раздел')
    await edit_or_answer(
        callback.message,
        f'📚 <b>{html.escape(title)}</b>\n\nВыбери запись:',
        reply_markup=character_sheet_entries_keyboard(entries, 'admin', section, character_id),
    )
    await callback.answer()


@router.callback_query(F.data.startswith('admin:sheet_entry:'))
async def admin_character_sheet_entry(callback: CallbackQuery, db: Database, settings: Settings) -> None:
    if await deny_if_not_admin(callback, settings):
        return
    _, _, character_id_raw, entry_id_raw = callback.data.split(':', 3)
    character_id = int(character_id_raw)
    entry = await db.get_character_sheet_entry(character_id, int(entry_id_raw))
    if entry is None:
        await callback.answer('Запись не найдена.', show_alert=True)
        return
    await edit_or_answer(
        callback.message,
        character_sheet_entry_text(entry),
        reply_markup=character_sheet_entry_keyboard('admin', entry['section'], character_id),
    )
    await callback.answer()


@router.callback_query(F.data.startswith('admin:toggle_inspiration:'))
async def admin_toggle_inspiration(callback: CallbackQuery, db: Database, settings: Settings) -> None:
    if await deny_if_not_admin(callback, settings):
        return
    character_id = int(callback.data.split(':')[2])
    success, result = await db.toggle_character_inspiration(character_id, callback.from_user.id)
    character = await db.get_character(character_id)
    if character:
        await edit_or_answer(
            callback.message,
...
@router.callback_query(F.data.startswith('admin:conditions:'))
async def admin_character_conditions(callback: CallbackQuery, db: Database, settings: Settings) -> None:
    if await deny_if_not_admin(callback, settings):
        return
    character_id = int(callback.data.split(':')[2])
    character = await db.get_character(character_id)
    if character is None:
        await callback.answer('Персонаж не найден.', show_alert=True)
        return
    conditions = await db.list_character_conditions(character_id)
    await edit_or_answer(
        callback.message,
        f'🏷️ <b>Состояния: {html.escape(character["display_name"])}</b>\n\n'
        f'Сейчас: <b>{format_conditions(conditions)}</b>\n\nНажатие добавляет или снимает состояние.',
        reply_markup=character_conditions_keyboard(character_id, conditions),
    )
    await callback.answer()


@router.callback_query(F.data.startswith('admin:condition_toggle:'))
async def admin_toggle_character_condition(callback: CallbackQuery, db: Database, settings: Settings) -> None:
    if await deny_if_not_admin(callback, settings):
        return
    _, _, character_id_raw, condition = callback.data.split(':')
    character_id = int(character_id_raw)
    was_active = condition in await db.list_character_conditions(character_id)
    success, result = await db.toggle_character_condition(character_id, condition, callback.from_user.id)
    character = await db.get_character(character_id)
    conditions = await db.list_character_conditions(character_id)
    if success and not was_active:
        await edit_or_answer(
            callback.message,
            f'Состояние добавлено. Как долго оно действует?',
            reply_markup=condition_duration_keyboard(character_id, condition),
        )
    elif character:
        await edit_or_answer(
            callback.message,
...
async def admin_edit_character_start(callback: CallbackQuery, state: FSMContext, db: Database, settings: Settings) -> None:
    ...
    labels = {
        ...
        'initiative_bonus': 'бонус инициативы от -30 до 30',
    }
    await state.clear()
...
@router.callback_query(F.data.startswith('admin:edit_character_notes:'))
async def admin_edit_character_notes_start(callback: CallbackQuery, state: FSMContext, db: Database, settings: Settings) -> None:
    if await deny_if_not_admin(callback, settings):
        return
    character_id = int(callback.data.split(':')[2])
    character = await db.get_character(character_id)
    ...
    await state.set_state(CharacterNotesState.notes)
    current = html.escape(str(character.get('admin_notes') or '').strip()) or 'пока пусто'
    await callback.message.answer(
        f'📝 <b>Заметки мастера для {character["display_name"]}</b>\n\n'
        f'Сейчас: {current}\n\n'
        'Отправь новый текст заметки. Чтобы очистить заметку, напиши /clear.'
    )
    await callback.answer()


@router.message(CharacterNotesState.notes, Command('clear'))
async def admin_character_notes_clear(message: Message, state: FSMContext, db: Database, settings: Settings) -> None:
    if await deny_if_not_admin(message, settings):
        return
    data = await state.get_data()
    character_id = int(data['character_id'])
    success, result = await db.update_character_notes(character_id, '', message.from_user.id)
    character = await db.get_character(character_id)
    await state.clear()
    await message.answer(
        result,
        reply_markup=admin_character_manage_keyboard(character) if character and success else back_to_admin_menu(),
    )


@router.message(CharacterNotesState.notes)
async def admin_character_notes_finish(message: Message, state: FSMContext, db: Database, settings: Settings) -> None:
    if await deny_if_not_admin(message, settings):
        return
    data = await state.get_data()
    character_id = int(data['character_id'])
    success, result = await db.update_character_notes(character_id, message.text.strip(), message.from_user.id)
    character = await db.get_character(character_id)
    await state.clear()
    if character is None:
        await message.answer('Персонаж не найден.', reply_markup=back_to_admin_menu())
...
@router.callback_query(F.data.startswith('admin:character_journal:'))
async def admin_character_journal(callback: CallbackQuery, db: Database, settings: Settings) -> None:
    if await deny_if_not_admin(callback, settings):
        return
    character_id = int(callback.data.split(':')[2])
    character = await db.get_character(character_id)
    if character is None:
        await callback.answer('Персонаж не найден.', show_alert=True)
        return
    entries = await db.list_character_journal(character_id, limit=20)
    await edit_or_answer(callback.message, journal_text(character, entries), reply_markup=admin_character_manage_keyboard(character))
    await callback.answer()
...
async def create_item_rarity(callback: CallbackQuery, state: FSMContext, settings: Settings) -> None:
    ...
    await state.set_state(CreateItemState.weight)
    await callback.message.answer('Введи вес одного предмета в кг. Например: <code>0.5</code>')
    await callback.answer()


@router.message(CreateItemState.weight)
async def create_item_weight(message: Message, state: FSMContext, settings: Settings) -> None:
    if await deny_if_not_admin(message, settings):
        return
    try:
        weight = float((message.text or '').strip().replace(',', '.'))
    except ValueError:
        await message.answer('Вес нужно ввести числом, например <code>0.5</code>.')
        return
    if weight < 0:
        await message.answer('Вес не может быть отрицательным.')
        return
    await state.update_data(weight_kg=weight)
    await state.set_state(CreateItemState.consumable)
    await message.answer('Предмет одноразовый?', reply_markup=create_consumable_keyboard())


@router.callback_query(CreateItemState.consumable, F.data.startswith('create_item:consumable:'))
async def create_item_consumable(callback: CallbackQuery, state: FSMContext, settings: Settings) -> None:
    if await deny_if_not_admin(callback, settings):
        return
    consumable = callback.data.rsplit(':', 1)[1] == 'yes'
    await state.update_data(is_consumable=consumable, heal_hp=0)
    await state.set_state(CreateItemState.equipment)
    await callback.message.answer('Можно ли экипировать предмет?', reply_markup=create_equipment_keyboard())
    await callback.answer()


@router.message(CreateItemState.heal_hp)
async def create_item_heal_hp(message: Message, state: FSMContext, settings: Settings) -> None:
    if await deny_if_not_admin(message, settings):
        return
    try:
        heal_hp = int((message.text or '').strip())
    except ValueError:
        await message.answer('Нужно целое число от 0.')
        return
    if heal_hp < 0:
        await message.answer('Лечение не может быть отрицательным.')
        return
    await state.update_data(heal_hp=heal_hp)
    await state.set_state(CreateItemState.equipment)
    await message.answer('Можно ли экипировать предмет?', reply_markup=create_equipment_keyboard())


@router.callback_query(CreateItemState.equipment, F.data.startswith('create_item:equipment:'))
async def create_item_equipment(callback: CallbackQuery, state: FSMContext, settings: Settings) -> None:
    if await deny_if_not_admin(callback, settings):
        return
    slot = callback.data.rsplit(':', 1)[1]
    await state.update_data(equipment_slot='' if slot == 'none' else slot, loot_chance_percent=0)
...
async def create_item_photo_wrong(message: Message, settings: Settings) -> None:
    if await deny_if_not_admin(message, settings):
        return
...
async def finish_item_creation(message: Message, state: FSMContext, db: Database, image_file_id: str | None) -> None:
    ...
    item_id = await db.create_item(
        ...
        equipment_slot=str(data.get('equipment_slot') or ''),
...
EDIT_FIELD_LABELS = {
    ...
    'weight_kg': 'вес в кг',
    'heal_hp': 'лечение HP при использовании',
...
async def admin_edit_item_value_finish(message: Message, state: FSMContext, db: Database, settings: Settings) -> None:
    ...
    if field in {'price', 'sell_price', 'heal_hp'}:
        try:
            ...
            await message.answer('Значение должно быть целым числом. Например: <code>10</code>')
            return
        if value < 0:
...
async def admin_edit_item_photo_wrong(message: Message, settings: Settings) -> None:
    if await deny_if_not_admin(message, settings):
        return
    await message.answer('Нужно отправить картинку как фото или написать /skip.')
...
@router.callback_query(F.data == 'admin:rests')
async def admin_rests(callback: CallbackQuery, db: Database, settings: Settings) -> None:
    if await deny_if_not_admin(callback, settings):
        return
    sessions = await db.list_sessions(only_active=True)
    if not sessions:
    ...
    await edit_or_answer(
        callback.message,
        '🛌 <b>Отдых</b>\n\nВыбери сессию. Отдых применится ко всем персонажам внутри неё и попадёт в журнал каждого персонажа.',
        reply_markup=rest_sessions_keyboard(sessions),
    )
    await callback.answer()


@router.callback_query(F.data.startswith('admin:rest_session:'))
async def admin_rest_session(callback: CallbackQuery, db: Database, settings: Settings) -> None:
    if await deny_if_not_admin(callback, settings):
        return
    session_id = int(callback.data.split(':')[2])
    session = await db.get_session(session_id)
    ...
    await edit_or_answer(
        callback.message,
        f'🛌 <b>Отдых</b>\n\nСессия: <b>{session["title"]}</b>\nПерсонажей: <b>{session.get("characters_count", 0)}</b>\n\n'
        'Выбери тип отдыха.',
        reply_markup=rest_type_keyboard(session_id),
    )
    await callback.answer()


@router.callback_query(F.data.startswith('admin:rest_type:'))
async def admin_rest_type(callback: CallbackQuery, state: FSMContext, db: Database, settings: Settings) -> None:
    if await deny_if_not_admin(callback, settings):
        return
    _, _, session_id_raw, rest_type = callback.data.split(':')
    session_id = int(session_id_raw)
    session = await db.get_session(session_id)
    if session is None:
        await callback.answer('Сессия не найдена.', show_alert=True)
        return
    await state.clear()
    await state.update_data(session_id=session_id, rest_type=rest_type)
    if rest_type == 'short':
        await state.set_state(RestState.short_hp)
        await callback.message.answer(
            f'Короткий отдых для сессии <b>{session["title"]}</b>.\n\n'
            'Сколько HP восстановить каждому персонажу? Например: <code>5</code>'
        )
    else:
        await state.set_state(RestState.note)
        await callback.message.answer(
            f'Долгий отдых для сессии <b>{session["title"]}</b> восстановит HP всех персонажей до максимума.\n\n'
            'Добавь заметку мастера для журнала или напиши /skip.'
        )
    await callback.answer()


@router.message(RestState.short_hp)
async def admin_rest_short_hp(message: Message, state: FSMContext, settings: Settings) -> None:
    if await deny_if_not_admin(message, settings):
        return
    try:
        hp_restore = int(message.text.strip())
    except ValueError:
        await message.answer('Нужно целое число HP. Например: <code>5</code>')
        return
    if hp_restore <= 0:
        await message.answer('Для короткого отдыха число HP должно быть больше 0.')
        return
    await state.update_data(hp_restore=hp_restore)
    await state.set_state(RestState.note)
    await message.answer('Добавь заметку мастера для журнала или напиши /skip.')


@router.message(RestState.note, Command('skip'))
async def admin_rest_note_skip(message: Message, state: FSMContext, db: Database, settings: Settings) -> None:
    if await deny_if_not_admin(message, settings):
        return
    await finish_rest(message, state, db, settings, note='')


@router.message(RestState.note)
async def admin_rest_note_finish(message: Message, state: FSMContext, db: Database, settings: Settings) -> None:
    if await deny_if_not_admin(message, settings):
        return
    await finish_rest(message, state, db, settings, note=message.text.strip())


async def finish_rest(message: Message, state: FSMContext, db: Database, settings: Settings, note: str) -> None:
    if await deny_if_not_admin(message, settings):
        return
    data = await state.get_data()
    success, result = await db.apply_session_rest(
        int(data['session_id']),
        str(data['rest_type']),
        hp_restore=int(data.get('hp_restore') or 0),
        note=note,
        admin_telegram_id=message.from_user.id,
    )
    await state.clear()
    await message.answer(result, reply_markup=back_to_admin_menu())
