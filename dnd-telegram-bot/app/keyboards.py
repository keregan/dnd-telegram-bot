...
def characters_keyboard(characters: list[dict], action: str) -> InlineKeyboardMarkup:
    ...
    for character in characters:
        builder.button(
            text=f'{character["display_name"]} | {character.get("session_title") or "Без сессии"} | ур. {character.get("level", 1)} | XP {character["xp"]} | 🪙 {character["gold"]}',
...
def admin_characters_keyboard(characters: list[dict]) -> InlineKeyboardMarkup:
    ...
    for character in characters:
        builder.button(
            text=f'{character["display_name"]} | {character.get("session_title") or "Без сессии"} | ур. {character.get("level", 1)} | 🪙 {character["gold"]}',
...
def admin_character_manage_keyboard(character: dict) -> InlineKeyboardMarkup:
    ...
    builder.button(text='🎲 Сессия', callback_data=f'admin:edit_character_session:{character_id}')
...
def inventory_item_keyboard(
    ...
    builder = InlineKeyboardBuilder()
    if can_use:
        builder.button(text='Использовать ×1', callback_data=f'inventory:use:{item_id}')
    if can_equip:
        if is_equipped:
...
def session_details_keyboard(session_id: int) -> InlineKeyboardMarkup:
    ...
    builder.button(text='🔔 Уведомить игроков', callback_data=f'admin:notify_session:{session_id}')
